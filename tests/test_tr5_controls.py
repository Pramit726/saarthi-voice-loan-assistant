...
from saarthi.domain.enums import ControlCommand, DeliveryStatus, JobKind, JobStatus, TurnRoute
...
def test_stop_cancels_active_work_and_queued_audio(conversation):
    ...
    stopped = ConversationStateMachine().apply_control(conversation, ControlCommand.STOP)
...
def test_pause_preserves_pending_field_and_revision(conversation):
    ...
    paused = ConversationStateMachine().apply_control(conversation, ControlCommand.PAUSE)
...
def test_repeat_does_not_replay_unheard_segment(conversation):
    ...
    repeated = ConversationStateMachine().apply_control(conversation, ControlCommand.REPEAT)
...
def test_go_back_changes_navigation_not_application_revision(conversation):
    ...
    changed = ConversationStateMachine().apply_control(conversation, ControlCommand.GO_BACK)
