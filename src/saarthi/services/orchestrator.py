...
from saarthi.domain.enums import ChangeKind, ControlCommand, DraftStatus, TurnAct, TurnRoute
...
class TurnOrchestrator:
    ...
    async def process_final_transcript(self, transcript: FinalTranscript) -> TurnOutcome:
        ...
        await self._trace(state, "final_transcript_accepted", "stt_gateway", "accepted", {
        ...
        try:
            ...
            return TurnOutcome(state=state, draft=draft, response_plan=plan, speech_segments=segments)

        state = self.state_machine.accept_proposal(state, proposal)
        await self.repository.save_state(state)
        await self._trace(state, "turn_interpreted", "tr1_interpreter", proposal.route.value, {
            "proposal_id": proposal.proposal_id,
            "acts": [act.value for act in proposal.acts],
            "target_field": proposal.target_field.value if proposal.target_field else None,
        ...
        if proposal.route is TurnRoute.CONTROL and proposal.control:
            ...
            if proposal.control is ControlCommand.CANCEL:
                draft = await self.repository.cancel_draft(draft.application_id, state.session_id)
            await self.repository.save_state(state)
            if proposal.control is ControlCommand.SHOW_SUMMARY and draft.current_projection:
                plan = self.renderer.projection_plan(draft.current_projection)
                plan = self.planner.guard.evaluate(plan, financial_response=True)
            else:
                plan = self.planner.for_control(proposal.control, repeat_text=repeat_text)
        ...
        elif proposal.route in {TurnRoute.CORRECTION} and proposal.rationale_code != "confirmed_pending_write":
            state = self.state_machine.hold_for_confirmation(state, proposal)
            await self.repository.save_state(state)
            current = draft.fields.get(proposal.target_field) if proposal.target_field else None
        ...
        elif proposal.route in {TurnRoute.FIELD_ANSWER, TurnRoute.CORRECTION} and proposal.explicit_write:
            is_correction = TurnAct.CORRECTION in proposal.acts or proposal.target_field in draft.fields
            patch = ApplicationPatch(
                ...
                change_kind=ChangeKind.CORRECTION if is_correction else ChangeKind.INITIAL,
            )
            ...
            if commit_result.accepted and commit_result.draft:
                ...
                try:
                    ...
                    pass
                state = self.state_machine.after_commit(state, set(draft.fields), draft.revision)
                await self.repository.save_state(state)
            plan = self.planner.for_commit(commit_result, next_field=state.pending_field)
            await self._trace(state, "application_patch_decided", "guarded_reducer", commit_result.reason_code, {
                "accepted": commit_result.accepted,
                "changed_field": commit_result.changed_field.value if commit_result.changed_field else None,
        ...
        elif proposal.route in {TurnRoute.FIELD_DOUBT, TurnRoute.PRODUCT_QUESTION, TurnRoute.CALCULATION}:
            if proposal.candidate_value is not None and proposal.target_field is not None:
                state = self.state_machine.hold_for_confirmation(state, proposal)
            ...
            resume = (
                ...
                else (state.resume_checkpoint.resume_prompt if state.resume_checkpoint else None)
            )
            plan = self.planner.for_grounded_answer(grounded_answer, resume_prompt=resume)
            if not state.pending_write_confirmation:
                state = self.state_machine.restore_checkpoint(state, draft.revision)
            await self.repository.save_state(state)
            await self._trace(state, "grounding_decided", "tr3_answer_service", grounded_answer.support_status.value, {
        ...
        if not plan.releasable:
            await self._trace(state, "response_blocked", "tr6_response_guard", "blocked", {
                "failed_gates": [gate.gate for gate in plan.release_gates if not gate.passed]
            })
        ...
        await self.repository.save_state(state)
        await self._trace(state, "response_released", "listener_renderer", "released", {
