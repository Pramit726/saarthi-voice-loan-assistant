...
class CachedKnowledgeProvider:
    ...
    @staticmethod
    def _key(query: str, product_id: str, product_version: str, fact_set_version: str, limit: int) -> str:
        material = "|".join(
            [query.casefold().strip(), product_id, product_version, fact_set_version, str(limit)]
        )
        ...
        *,
        ...
        if self.redis:
            try:
                ...
                if cached:
                    results = [RetrievedFact.model_validate(item) for item in json.loads(cached)]
